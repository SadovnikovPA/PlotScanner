from math import hypot, sqrt, pi, exp
...
def get_gaussian_kernel(radius, sigma, normalized=True):
    ...
    if normalized:
        result_kernel = np_normalize(result_kernel)
...
def get_gaussian_derivative(radius, sigma, derivative_kernel, normalized=True):
    ...
    if normalized:
        gaussian_derivative = np_normalize(gaussian_derivative)
...
def get_gaussian_laplasian(radius, sigma, derivative_kernel, normalized=True):
    ...
    if normalized:
        gaussian_laplasian = np_normalize(gaussian_laplasian)
    return gaussian_laplasian


def np_normalize(kernel):
...
def normalize(kernel):
    normalizer_positive = sum(sum([item for item in string if item > 0]) for string in kernel)
    normalizer_negative = sum(sum([item for item in string if item < 0]) for string in kernel)
    if normalizer_positive == 0 and normalizer_negative == 0:
        return kernel #nothing to normalize: only zeroes
    normalizer = 1/(normalizer_positive - normalizer_negative)
    kernel = [[normalizer*item for item in string] for string in kernel]
    return kernel
