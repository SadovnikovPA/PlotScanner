...
def compute_integral_image_buffer(pix, height, width, mode = 1):
    ...
    """
    integral_image_buffer = np.zeros((height, width))

    if mode == 1:
        integral_image_buffer[0, 0] = pix[0, 0]
        for x in range(1, width):
            integral_image_buffer[0, x] = integral_image_buffer[0, x-1] + pix[x, 0]
        ...
        for x in range(1, width):
            integral_image_buffer[0, x] = integral_image_buffer[0, x - 1] + (pix[x, 0][0] + pix[x, 0][1] + pix[x, 0][2])/3
        for y in range(height):
            for x in range(width):
...
def piece_of_integral_image_buffer(integral_image_buffer, width, height, x1, y1, x2, y2):
    ...
    return integral_image_buffer[y2, x2] + integral_image_buffer[y1, x1] - \
                    integral_image_buffer[y2, x1] - integral_image_buffer[y1, x2]